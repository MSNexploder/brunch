(function() {
  var banner, brunch, colors, dsl, fs, globalOpts, helpers, options, parser, path, usage, version, yaml, _;
  ...
  exports.loadConfigFile = function(configPath, options) {
    var opts, _ref;
    try {
      opts = yaml.eval(fs.readFileSync(configPath, 'utf8'));
      if ((_ref = options.stitch) == null) {
        options.stitch = {};
      }
      if (opts.dependencies != null) {
        options.stitch.dependencies = opts.dependencies;
      }
      if (opts.templateExtension != null) {
        options.stitch.templateExtension = opts.templateExtension;
      }
      if (opts.minify != null) {
        options.stitch.minify = opts.minify;
      }
