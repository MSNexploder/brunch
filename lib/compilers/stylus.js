(function() {
  var Compiler, brunch, colors, fs, helpers, nib, path, stylus;
  ...
  exports.StylusCompiler = (function() {
    ...
    StylusCompiler.prototype.filePattern = function() {
      return this.options.stylus.filePattern;
    };
    StylusCompiler.prototype.compile = function(files) {
      var mainFilePath;
      mainFilePath = path.join(this.options.rootPath, 'src/app/styles/main.styl');
      return fs.readFile(mainFilePath, 'utf8', __bind(function(err, data) {
        var compiler;
        if (err != null) {
          return helpers.log(colors.lred('stylus err: ' + err));
        } else {
          compiler = stylus(data).set('filename', mainFilePath).set('compress', true).include(path.join(this.options.rootPath, 'src'));
          if (nib) {
            compiler.use(nib);
          }
          return compiler.render(__bind(function(err, css) {
            if (err != null) {
              ...
              return fs.writeFile(this.options.stylus.output, css, 'utf8', __bind(function(err) {
                if (err != null) {
                  return helpers.log(colors.lred('stylus err: ' + err));
                } else {
