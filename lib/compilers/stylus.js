(function() {
  ...
  exports.StylusCompiler = (function() {
    ...
    StylusCompiler.prototype.compile = function(files) {
      var mainFilePath;
      mainFilePath = this.generatePaths('src/app/styles/main.styl');
      return fs.readFile(mainFilePath, 'utf8', __bind(function(err, data) {
        var compiler;
        if (err != null) {
          return helpers.log(colors.lred('stylus err: ' + err));
        } else {
          compiler = stylus(data).set('filename', mainFilePath).set('compress', true).include(this.generatePaths('src/'));
          if (nib) {
            compiler.use(nib);
          }
          return compiler.render(__bind(function(err, css) {
            if (err != null) {
              ...
              return this.writeToFile(this.options.output, css, __bind(function(err) {
                if (err != null) {
                  return helpers.log(colors.lred('stylus err: ' + err));
                } else {
